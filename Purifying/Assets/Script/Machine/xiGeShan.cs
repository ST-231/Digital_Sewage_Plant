...
public class XiGeShan : MonoBehaviour
{
    [Header("输入参数")]
    public float Q; //最大设计流量
    private float k = 70f; //水头损失增大倍数
    private float beta = 1.79f; //形状系数，取圆形栅条截面形状1.79
    private float n = 202f; //栅条间隙数
    private float a = 60f; //格栅倾角

    [Header("可调参数")]
    private float S = 0.01f; //栅条宽度
    private float b = 0.005f; //栅条间隙
    ...
    void OnEnable()
    {
        ...
        bSlider.minValue = 0.005f;
        bSlider.maxValue = 0.05f;
        bSlider.value = b;

        QSlider.maxValue = WaterData.Instance.GetQSave(0);
    ...
    void UpdateParameterDisplay()
    {
        ...
        {
            if (v < 1.0f && h1 < 1.0f && WaterData.Instance.GetCuGeshan()&&Q<Eff)
            {
                WaterData.Instance.SetProgress(20.0f);
                WaterData.Instance.SetXiGeshan(true);
            }
            else
            {
                WaterData.Instance.SetProgress(10.0f);
                WaterData.Instance.SetXiGeshan(false);
            }
        }
    ...
    {
        Q = newValue;
        WaterData.Instance.SetQSave(1, Q);
        Debug.Log("QSaved:" + WaterData.Instance.GetQSave(1));
        CalculateParameters();
        UpdateParameterDisplay();
    }
    ...
    void CalculateParameters()
    {
        ...
        B = S * (n - 1) + (b * n);
        v = ((Q/10000f) * Mathf.Sqrt(Mathf.Sin(radians)) / (b * h * n));
        ...
        Cost = E * 0.6f * Q;
        WaterData.Instance.setCost(Cost, 1);
